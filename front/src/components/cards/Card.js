...
const Card = ({ data, index,setisInside }) => {
  ...
  const OnDelete = async e => {
    try {
      e.preventDefault();
      const getUser = await axios.get(`http://localhost:5000/route/note/${data._id}`);
      if(getUser.parent == null){
      }

      const res = await axios.delete(`http://localhost:5000/route/note/${data._id}`);
      console.log(res.data , "deleted data")
      var d = res.data
      if (d.parent == null) {
          //craete axios request to remove it from user
        const par = await axios.get(`http://localhost:5000/route/user/userinfo`)
        // Element to remove
        var c = par.notes
        const elementToRemove = d._id;
        c = c.filter(item => item !== elementToRemove);
        const rem = await axios.put(`http://localhost:5000/route/user/updateUser`, { notes: c });
      }
      else {
        const par = await axios.get(`http://localhost:5000/route/note/${d._id}`)
        // Element to remove
        var c = par.children
        const elementToRemove = d._id;
        c = c.filter(item => item !== elementToRemove);
        const rem = await axios.put(`http://localhost:5000/route/note/${d.parent}`, { children: c });
      }
  ...
  return (
    ...
    >
      ...
      <div className="task-holder">
        <Link to={`/note/${data._id}` } onClick = {e=> {
        ...
        {/* <div className="task-holder">
          <Link to={`/note/${data._id}` } key = {index}>
              <span
              className="card-header"
              style={{
                backgroundColor: colors[index % 5].secondaryColor,
        ...
        {/* <div style={{ position: "absolute", right: "4px", bottom: "6px", width: "95%" }}>
          <button type="button" ><p style={{ all: "unset", left: "0px" }}>delete</p></button>
          <button type="button" ><p style={{ all: "unset", left: "0px" }}>Edit</p></button>
        </div> */}
...
<path d="M 10 2 L 9 3 L 4 3 L 4 5 L 5 5 L 5 20 C 5 20.522222 5.1913289 21.05461 5.5683594 21.431641 C 5.9453899 21.808671 6.4777778 22 7 22 L 17 22 C 17.522222 22 18.05461 21.808671 18.431641 21.431641 C 18.808671 21.05461 19 20.522222 19 20 L 19 5 L 20 5 L 20 3 L 15 3 L 14 2 L 10 2 z M 7 5 L 17 5 L 17 20 L 7 20 L 7 5 z M 9 7 L 9 18 L 11 18 L 11 7 L 9 7 z M 13 7 L 13 18 L 15 18 L 15 7 L 13 7 z"></path>
            ...
            <Link to = {`/edit/${data._id}`} style={{cursor:"pointer"}}>
